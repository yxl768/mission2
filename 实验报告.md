...
> 结果诚信说明：下述数值均为2026-07-15至2026-07-21在本机上的实际运行结果。由于PubChem API限制和OPSIN工具安装失败（缺少Java环境），本实验仅获得206个带有IUPAC名称的唯一分子，并不把这次结果外推为完整ZINC-250K训练结果。所有数据均可通过运行项目代码复现。
